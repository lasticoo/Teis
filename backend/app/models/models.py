...
class EdgeBlueprint(Base):
    ...
    win_rate_ci_lower = Column(Numeric(5, 2), nullable=True)
    win_rate_ci_upper = Column(Numeric(5, 2), nullable=True)
    p_value = Column(Numeric(10, 6), nullable=True)
    fdr_adjusted_p_value = Column(Numeric(10, 6), nullable=True)
    is_fdr_significant = Column(Boolean, default=False)
    out_of_sample_expectancy_r = Column(Numeric(8, 4), nullable=True)
    is_stable = Column(Boolean, nullable=True)
    is_repeatable = Column(Boolean, nullable=True)
    is_robust = Column(Boolean, nullable=True)
    stability_detail = Column(JSON, nullable=True)
    repeatability_detail = Column(JSON, nullable=True)
    robustness_detail = Column(JSON, nullable=True)
    criteria_evaluated_at = Column(DateTime(timezone=False), nullable=True)
